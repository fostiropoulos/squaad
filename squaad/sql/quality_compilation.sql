WITH
  ...
  ( SELECT
    ...
    quality_metrics as (
      SELECT
        locs.email         AS email,
        total.organization as organization,
  ...
  email_app_group_by_impactful AS (select
                                        cs.email                                                        as email,
                                        cs.application                                                  as app,
                                        lower(SUBSTRING(cs.email, (position('@' in cs.email) + 1),
                                                        2500))                                          as domain,
...
SELECT
quality_metrics.email         AS email,
        quality_metrics.organization as organization,
        quality_metrics.domain       as domain,
        quality_metrics.app           as app,
        --email_app_group_by_impactful.domain as domain,
        --email_app_group_by_impactful.organization as organization,
        --email_app_group_by_impactful.c_analyzed as c_analyzed,
        --email_app_group_by_impactful.c_impactful as c_impactful,
        quality_metrics.loc           AS loc,
        quality_metrics.cpx          AS cpx,
        quality_metrics.sml           AS sml,
        quality_metrics.vul           AS vul,
        quality_metrics.fbg           AS fbg,
        quality_metrics.locs_inc       AS locs_inc,
        quality_metrics.cplxs_inc      AS cplxs_inc,
        quality_metrics.smls_inc       AS smls_inc,
        quality_metrics.vuls_inc       AS vuls_inc,
        quality_metrics.fbgs_inc       AS fbgs_inc,
        quality_metrics.locs_dec       AS locs_dec,
        quality_metrics.cplxs_dec      AS cplxs_dec,
        quality_metrics.smls_dec       AS smls_dec,
        quality_metrics.vuls_dec       AS vuls_dec,
        quality_metrics.fbgs_dec       AS fbgs_dec,
        quality_metrics.total          AS total,
        email_app_group_by_impactful.c_impactful as impactful,
        email_app_group_by_impactful.c_analyzed as analyzed

FROM quality_metrics  JOIN email_app_group_by_impactful ON quality_metrics.email = email_app_group_by_impactful.email AND quality_metrics.app = email_app_group_by_impactful.app
